...
def test_well_modelled_scores_high():
    result = DataOverPrimitives().evaluate(_codebase(WELL_MODELLED), DataOverPrimitives.Params())
...
def test_primitive_heavy_scores_low():
    result = DataOverPrimitives().evaluate(_codebase(PRIMITIVE_HEAVY), DataOverPrimitives.Params())
    assert result.status == "measured"
...
def test_well_beats_primitive():
    well = DataOverPrimitives().evaluate(_codebase(WELL_MODELLED), DataOverPrimitives.Params())
    prim = DataOverPrimitives().evaluate(_codebase(PRIMITIVE_HEAVY), DataOverPrimitives.Params())
    assert well.score_contribution > prim.score_contribution


def test_no_annotations_is_not_measured():
    result = DataOverPrimitives().evaluate(_codebase("def f(a, b):\n    return a + b\n"), DataOverPrimitives.Params())
    assert result.status == "not_measured"


def test_private_and_dunder_excluded():
    text = "class C:\n    def _hidden(self, x: str) -> str: ...\n    def __eq__(self, o: object) -> bool: ...\n"
    result = DataOverPrimitives().evaluate(_codebase(text), DataOverPrimitives.Params())
...
def test_vocab_density_denominator_excludes_test_files():
    ...
    result = DataOverPrimitives().evaluate(cb, DataOverPrimitives.Params())
