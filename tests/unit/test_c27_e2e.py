...
def test_fixture_ranking():
    well = DataOverPrimitives().evaluate(load_codebase(FIXTURES / "well_modelled"), DataOverPrimitives.RuleConfig())
    prim = DataOverPrimitives().evaluate(load_codebase(FIXTURES / "primitive_heavy"), DataOverPrimitives.RuleConfig())
