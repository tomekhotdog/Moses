...
Each Commandment produces a `CommandmentResult` with a `score_contribution`
in [0, 100] and a `status`. The overall Score is a weighted mean over the rules
that are **both enabled and measured**:
